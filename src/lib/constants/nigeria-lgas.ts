/**
 * Nigeria's administrative dataset: 37 states (36 + FCT) and all 774 Local
 * Government Areas. Ported verbatim from
 * `app-rateo/src/constants/nigeriaStatesLGA.js`, which is the authority the
 * mobile company-KYC screen writes `state` / `city` from - the two must agree
 * or an admin reviewing a business submission sees an LGA that does not exist
 * in the state it was filed under.
 *
 * `src/lib/constants/locations.ts` keeps the separate, user-facing state list
 * used for `user.location` (it spells the capital territory "FCT - Abuja");
 * this file spells it "FCT" because that is the key the LGA map is stored
 * under. Do not merge them.
 */

/** State name -> its LGAs, alphabetical within each state. */
export const NIGERIA_STATES_LGAS: Record<string, readonly string[]> = {
  ...
  "Adamawa": [
    "Demsa", "Fufure", "Ganye", "Gayuk", "Gombi", "Grie", "Hong", "Jada", "Lamurde", "Madagali",
  ...
  "Ogun": [
   ...
   "Obafemi Owode", "Odeda", "Odogbolu", "Ogun Waterside", "Remo North", "Sagamu", "Yewa North",
   "Yewa South",
  ],
  "Ondo": [
  ...
  "Sokoto": [
   ...
   "Kebbe", "Kware", "Rabah", "Sabon Birni", "Shagari", "Silame", "Sokoto North",
...
/** The 37 state names, sorted. */
export const NIGERIA_KYC_STATES: readonly string[] = Object.keys(NIGERIA_STATES_LGAS).sort();

/** Chosen when the state is not in the list - the user types both fields. */
export const STATE_OTHER = 'Other (type it myself)';
/** Chosen for a business registered outside Nigeria - the LGA field is free text. */
export const STATE_ABROAD = 'Abroad';
/** Offered inside every state's LGA list, for LGAs the dataset is missing. */
export const LGA_OTHER = 'Other (type your LGA)';

/** LGAs for a state; empty for an unknown or custom state name. */
